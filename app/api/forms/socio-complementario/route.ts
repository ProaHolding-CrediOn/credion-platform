...
// Definición del formulario del SOCIO-Complementario: se reutiliza la MISMA
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('Unexpected error in GET /api/forms/socio-complementario', error)
...
export async function POST(req: NextRequest) {
  try {
    const token = req.headers.get('Authorization') as string
    const { creditId, formComplementarioSocio, version } = await req.json()

    const response = await fetch(
      `${process.env.CORE_SERVICE_API_URL}/credit/form-socio-complementario`,
      {
        method: 'POST',
        headers: {
          Authorization: token,
        ...
        body: JSON.stringify({ creditId, formComplementarioSocio, version }),
      },
    )
    ...
    console.error('Unexpected error in POST /api/forms/socio-complementario', error)
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 })
  }
}
