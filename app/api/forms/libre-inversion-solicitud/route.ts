...
/**
 * Definición y envío del formulario de solicitud de crédito de libre inversión.
 *
 ...
 */
const FORM_KEY = 'solicitud-libre-inversion'
...
export async function GET() {
  try {
    ...
    console.error('Unexpected error in GET /api/forms/libre-inversion-solicitud', error)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${process.env.CORE_SERVICE_API_URL}/credit/form-libre-inversion-solicitud`,
      {
        method: 'POST',
        headers: { Authorization: token, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          creditId: body.creditId,
          formSolicitudLibreInversion: body.formSolicitudLibreInversion,
    ...
    console.error('Unexpected error in POST /api/forms/libre-inversion-solicitud', error)
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 })
  }
}
